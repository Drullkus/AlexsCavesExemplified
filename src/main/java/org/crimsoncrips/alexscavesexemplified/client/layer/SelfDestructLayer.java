...
public class SelfDestructLayer extends RenderLayer<NotorEntity, NotorModel> {
    ...
    public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, NotorEntity pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if (destructTime / (5 * (-destructTime + 20)) % 2 == 0){
            System.out.println("Test");
            System.out.print(((ACEBaseInterface)pLivingEntity).getSelfDestructTime());
            VertexConsumer vertexconsumer = pBuffer.getBuffer(getParentModel().renderType(SELF_DESTRUCT_LAYER));
            ...
            entitymodel.renderToBuffer(pPoseStack, vertexconsumer, pPackedLight, OverlayTexture.NO_OVERLAY, 1F, 1F, 1F, 0.4F);
